...
const FOOTER_COLS = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: "About Ulnar Medical", href: "/about" },
    ],
  },
];
...
const SOCIAL_LINKS = [
  ...
  { label: "Facebook", href: "#", color: "#1877F2", live: false },
  { label: "Instagram", href: "#", color: "#E4405F", live: false },
  { label: "TikTok", href: "#", color: "#ffffff", live: false },
];
...
function SocialIcon({ item, index }: { item: (typeof SOCIAL_LINKS)[number]; index: number }) {
  ...
  return (
    <motion.a
      ...
      className="relative w-11 h-11 rounded-full flex items-center justify-center overflow-hidden border border-[rgba(255,255,255,0.1)] cursor-pointer"
      style={{ background: "rgba(255,255,255,0.04)", opacity: item.live ? 1 : 0.45 }}
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 md:px-14 py-16 md:py-24">
        ...
        {/* Top — logo + contact */}
        <AnimatedContainer className="flex flex-col md:flex-row md:items-center justify-between gap-10 pb-14 border-b border-[rgba(255,255,255,0.08)]">
          <Logo size={48} />
          <div className="flex flex-col md:items-end gap-2">
            <span className="font-mono text-[10px] uppercase tracking-widest text-[rgba(248,246,242,0.4)]">
              Direct Clinic Lines
            </span>
            <a
              href="tel:+254724273996"
              className="font-serif italic font-semibold text-[#FFD43A] hover:text-white transition-colors duration-300"
              style={{ fontFamily: "var(--font-cormorant), serif", fontSize: "clamp(1.3rem, 2.2vw, 1.7rem)" }}
            >
              +254 724 273 996
            </a>
            <a
            ...
            <a
              ...
              className="font-sans text-[rgba(248,246,242,0.45)] text-sm hover:text-[#F4B9B9] transition-colors duration-300"
            >
        ...
        {/* Nav columns */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 pt-12 pb-14">
          {FOOTER_COLS.map((col, index) => (
            <AnimatedContainer key={col.title} delay={0.1 + index * 0.1}>
              <div>
                <h3 className="font-mono text-[10px] uppercase tracking-widest text-[#F4B9B9] mb-5">
                  {col.title}
                </h3>
                <ul className="flex flex-col gap-3">
                  {col.links.map((link) => (
                    <li key={link.label}>
                      <motion.a
                        href={link.href}
                        className="font-sans text-[rgba(248,246,242,0.45)] text-sm hover:text-[#FFD43A] transition-colors duration-300 inline-block relative group"
        ...
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8 pt-8 border-t border-[rgba(255,255,255,0.07)]">
          ...
          <AnimatedContainer delay={0.5}>
            <div
              className="font-serif italic font-bold select-none leading-[0.85]"
              style={{
            ...
            <p
              ...
              style={{ fontSize: "clamp(0.5rem, 0.9vw, 0.72rem)", letterSpacing: "0.26em", color: "rgba(248,246,242,0.22)" }}
            >
              Diagnostic Centre
            </p>
            <p className="font-sans text-[rgba(248,246,242,0.35)] text-xs mt-3 tracking-wide">
              ©{new Date().getFullYear()} Ulnar Medical and Diagnostic Centre. All rights reserved.<br />
              Ngong Road, Nairobi, Kenya.
            </p>
          </AnimatedContainer>

          {/* Social icons */}
          <AnimatedContainer delay={0.6} className="flex flex-col items-start md:items-end gap-3">
            <span className="font-mono text-[10px] uppercase tracking-widest text-[rgba(248,246,242,0.3)]">
              Patient Digital Gateways
            </span>
