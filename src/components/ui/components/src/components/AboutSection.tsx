...
function ImageTrail({
  ...
  const activate = (el: HTMLImageElement, x: number, y: number) => {
    ...
    if (fadeAnimation) {
      setTimeout(() => { el.dataset.status = "inactive"; }, 1500);
    }
    lastPos.current = { x, y };
  };

  const deactivate = (el: HTMLImageElement) => {
    el.dataset.status = "inactive";
  };

  const getDistance = (x: number, y: number) =>
    Math.hypot(x - lastPos.current.x, y - lastPos.current.y);
  ...
  return (
    <section className="relative bg-[#080f1e] pt-24 pb-16 overflow-hidden border-t border-white/5">
      <div
        ...
        style={{ width: 600, height: 600, background: "radial-gradient(circle, rgba(255,212,58,0.05) 0%, transparent 65%)" }}
        aria-hidden
      />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <div className="flex-shrink-0 w-10 h-10 rounded-full bg-[rgba(255,212,58,0.12)] flex items-center justify-center">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#FFD43A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
