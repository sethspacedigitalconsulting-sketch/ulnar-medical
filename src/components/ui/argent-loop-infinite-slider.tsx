...
export function InfiniteParallaxSlider() {
  ...
  /* ── Cursor-chase refs for escape buttons ── */
  ...
  const animationLoop = React.useCallback(() => {
    ...
    /* ── Cursor-chase for escape buttons ── */
  ...
  React.useEffect(() => {
    ...
    if (el) {
      ...
      el.addEventListener("mouseleave", () => {
        isHovered.current = false;
        btnTargetX.current = 0;   // return buttons to centre on mouse exit
      });
    }
  ...
  return (
    ...
    >
      ...
      {/* ── Escape up: Previous Section — follows cursor horizontally ── */}
      <button
        ref={prevBtnRef}
      ...
      {/* ── Escape down: Next Section — follows cursor horizontally ── */}
      <button
        ref={nextBtnRef}
